...
/* eslint-disable no-underscore-dangle, no-param-reassign, no-shadow */
...
function treemapViz(slice) {
  const div = d3.select(slice.selector);
  const draw = function (data, eltWidth, eltHeight, formData) {
    ...
    const text2 = function (selection) {
      selection.attr('x', (d) => x(d.x + d.dx) - this.getComputedTextLength() - 6)
  ...
  const render = function () {
    d3.json(slice.jsonEndpoint(), function (error, json) {
      ...
      for (let i = 0, l = json.data.length; i < l; i ++) {
        draw(json.data[i], width, height, json.form_data);
      }
      slice.done(json);
    });
  };
